...
DETAILS = {
    ...
    "morning_exercise": [
        "热身 3 min：开合跳 30 s → 猫牛式 10 次 → 髋环绕 10 次/侧",
        "主训 20 min（核心 / 臀 / 上肢 3 轮）",
        "放松 2–3 min 拉伸 + 洗漱"
    ],
    "am": [
        "主学习 (10:00–13:00)：CS / 算法 / 项目推进",
        "番茄 50/10 模式，两轮后远眺 1–2 min"
    ],
    "noon": [
        "午睡 15–20 min（设闹钟）",
        "起床即动：床上瘦/臀腿 / 快走 10–15 min",
        "补水 200 ml → 清醒模式"
    ],
    "pm": [
        "学术专注 (14:00–18:00)：CS / 论文 / 编码",
        "每 90 min 起身 5 min（倒咖啡 / 热茶）"
    ],
    "evening": [
        "18:00 晚餐：蛋白 + 蔬菜 + 少量主食（七分饱）",
        "饭后 10 min 散步或收拾台面",
        "19:00–21:45 自由时间：电影 / 剧 / 轻课程 / 写日记"
    ],
    "night_prepare": [
        "21:45 洗脸 + 护肤；（周三/周日）洗澡洗头 → 护肤 → 吹干",
        "轻音乐 / 呼吸 10 min → 入睡"
    ]
}
...
for idx, block in enumerate(st.session_state["blocks"]):
    ...
    dkey = block["key"]
    if dkey in DETAILS:
        with st.expander("细节清单", expanded=True):
